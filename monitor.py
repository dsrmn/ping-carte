...
def check_card(client, card_name, blueprint_entries, criteria, min_listings, sent_alerts, allowed_seller_countries, max_expansion_year, trim_pct):
...
def main():
    ...
    for card_name, blueprint_entries in cards_index.items():
        if not blueprint_entries:
            continue
        deals = check_card(client, card_name, blueprint_entries, criteria, min_listings, sent_alerts, allowed_seller_countries, max_expansion_year, trim_pct)

        for deal in deals:
            msg = (
                f"🃏 <b>Occasione trovata!</b>\n"
                f"<b>{deal['card_name']}</b> ({deal['expansion']})\n"
                f"Prezzo: <b>{deal['price']:.2f}€</b> (riferimento: {deal['avg_price']:.2f}€)\n"
