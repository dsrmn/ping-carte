...
def check_card(client, card_name, blueprint_entries, criteria, min_listings, sent_alerts, allowed_seller_countries, restricted_rarity_from_year, allowed_rarities, trim_pct, min_condition_rank):
    ...
    for entry in blueprint_entries:
        ...
        # Espansioni dall'anno soglia in poi: solo le rarità in allowed_rarities.
        # Prima di quell'anno (o se l'anno non è noto): nessun filtro rarità.
        require_rarity_filter = restricted_rarity_from_year is not None and exp_year is not None and exp_year >= restricted_rarity_from_year
        try:
...
def main():
    ...
    restricted_rarity_from_year = config.get("restricted_rarity_from_year")
    ...
    for i, (card_name, blueprint_entries) in enumerate(cards_index.items(), 1):
        ...
        if not blueprint_entries:
            continue
        deals = check_card(client, card_name, blueprint_entries, criteria, min_listings, sent_alerts, allowed_seller_countries, restricted_rarity_from_year, allowed_rarities, trim_pct, min_condition_rank)
