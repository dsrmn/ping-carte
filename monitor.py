...
def check_card(client, card_name, blueprint_entries, criteria, min_listings, sent_alerts, allowed_seller_countries, foil_only_from_year, trim_pct, min_condition_rank):
    ...
    for entry in blueprint_entries:
        ...
        # Espansioni dall'anno soglia in poi: solo foil (qualsiasi rarità va
        # bene). Prima di quell'anno (o se l'anno non è noto): nessun filtro
        # foil, qualsiasi versione va bene.
        require_foil = foil_only_from_year is not None and exp_year is not None and exp_year >= foil_only_from_year
        try:
            # language="en" filtra lato API solo le inserzioni in inglese
            products = client.get_marketplace_products(
                bp_id, language="en", foil=True if require_foil else None
            )
...
def main():
    ...
    foil_only_from_year = config.get("foil_only_from_year")
    ...
    for i, (card_name, blueprint_entries) in enumerate(cards_index.items(), 1):
        ...
        if not blueprint_entries:
            continue
        deals = check_card(client, card_name, blueprint_entries, criteria, min_listings, sent_alerts, allowed_seller_countries, foil_only_from_year, trim_pct, min_condition_rank)
